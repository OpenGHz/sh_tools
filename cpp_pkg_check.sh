...
if [ $# -ne 1 ]; then
    ...
    echo "Example: $0 opencv"
    echo "Note: <pkg_name> can be part of the whole name."
    exit 1
fi
...
if [ -e /usr/include/$PKG_NAME ]; then
    echo "Found $PKG_NAME .h files in /usr/include/"
else
    echo "Not found $PKG_NAME .h files in /usr/include/"
fi

if [ -e /usr/local/include/$PKG_NAME ]; then
    echo "Found $PKG_NAME .h files in /usr/local/include/"
else
    echo "Not found $PKG_NAME .h files in /usr/local/include/"
fi

echo ""
echo "Check '.so' files:"
if ! ldconfig -p |grep $PKG_NAME; then
    echo "Not found $PKG_NAME .so files in /lib, /usr/lib and /etc/ld.so.conf"
fi

echo ""
echo "Check '.a' files:"
if ! find /usr/lib -name '*.a' |grep $PKG_NAME && ! find /usr/local/lib -name '*.a' |grep $PKG_NAME; then
    echo "Not found $PKG_NAME .a files in /usr/lib/ and /usr/local/lib/"
fi
